...
double ldrop_crust::shell_energy(double Z, double N, 
...
(double Z, double N, double npout, double nnout, double chi, double T) {
  ...
  if (chi<0.0) {
    O2SCL_ERR2("Variable 'chi' is less than zero in ",
	       "ldrop_crust::drip_binding_energy_full_d().",o2scl::exc_einval);
    return 0.0;
  }
...
double ldrop_crust::nucleus_be(int Z, int N, double npout, double nnout, 
...
ldrop_crust::ldrop_solve_chi::ldrop_solve_chi
(ldrop_crust *tp, int Z, int N, double np, double nn, double T, double ne) {
  tptr=tp;
  Z_=Z;
  N_=N;
  np_=np;
...
double ldrop_crust::ldrop_solve_chi::operator()(double chi) const {
  
  // Compute the nucleus
  tptr->drip_binding_energy_full_d(Z_,N_,np_,nn_,chi,T_)/
    o2scl_const::hc_mev_fm;
...
ldrop_crust::ldrop_be_deriv::ldrop_be_deriv
(ldrop_crust &ld, double Z, double N, double npout, double nnout,
...
double ldrop_crust::ldrop_be_deriv::operator()(double x) {
  if (jx_==0) {
    Z_=x;
  } else if (jx_==1) {
    N_=x;
